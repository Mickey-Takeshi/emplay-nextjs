...
const services = [
  { number: '01', label: 'HP PRODUCTION', title: 'HP制作サービス', description: '企業の信頼性と問い合わせ獲得を両立するホームページを、SEO・AIOを踏まえて設計。公開後の改善まで伴走します。', href: '/service/hp', pricing: '初期費用45万円から', image: '/images/renewal/service-hp-wide.webp', alt: 'ホームページを構成するレイヤーの図解' },
  { number: '02', label: 'CREATIVE', title: 'クリエイティブ制作', description: 'Webサイト、LP、バナー、動画など、目的と利用場面に合わせたクリエイティブを企画・制作します。', href: '/service/creative', pricing: '料金は要問い合わせ', image: '/images/renewal/service-creative-wide.webp', alt: '制作机上のクリエイティブ工程の図解' },
  { number: '03', label: 'ADVERTISING', title: 'Web広告運用代行', description: '媒体選定から配信設計、クリエイティブ改善、効果検証まで一貫して支援します。', href: '/service/ads', pricing: '料金は要問い合わせ', image: '/images/renewal/service-ads-wide.webp', alt: '広告流入を成果へつなげるファネルの図解' },
  { number: '04', label: 'CRM', title: 'CRM導入支援', description: '顧客情報の整理からツール選定、初期設定、配信・運用まで、社内で継続活用できる状態をつくります。', href: '/service/crm', pricing: '料金は要問い合わせ', image: '/images/renewal/service-crm-wide.webp', alt: '顧客接点をつなぐCRM導線の図解' },
  { number: '05', label: 'AI TRAINING', title: 'EMPLAY AI ACADEMY', description: '生成AIを実務で使い、自走できる人材を育てる企業向けオンライン研修です。', href: 'https://academy.emplay.jp/', pricing: '研修プログラムを見る ↗', image: '/images/renewal/service-academy-wide.webp', alt: '生成AIを学ぶ円形ワークショップの図解', external: true },
]
...
export default function ServiceClient() {
  ...
  return <main className="service-page">
    <FaqJsonLd faqs={faqs} />
    <header className="service-hero"><div className="service-hero-bg"><img src="/images/renewal/ecosystem.webp" srcSet={SRCSET("/images/renewal/ecosystem.webp")} sizes="100vw" alt="5つの専門領域を連携させる支援体制の図解" fetchPriority="high" decoding="async" style={{position:'absolute',inset:0,width:'100%',height:'100%'}} /></div><div className="service-hero-wash" aria-hidden="true"/><div className="container service-hero-content"><p className="service-hero-label">Services</p><h1>必要な施策を選び、<br />ひとつの実行計画へ。</h1><p>HP制作、クリエイティブ、Web広告、CRM、AI研修。各領域を単独でご相談いただくことも、課題に合わせて組み合わせることもできます。</p><div className="service-hero-actions"><Link href="/contact" className="btn btn-cta">無料相談を申し込む</Link><a href="#services" className="btn btn-outline">サービスを見る ↓</a></div></div></header>
    ...
    <section className="service-hub-section" id="services"><div className="service-wide"><div className="container service-section-heading"><div><p className="section-label">Service Lineup</p><h2>5つの専門領域</h2></div><p>サービスごとに異なる専門性を、共通の進め方と窓口で提供します。</p></div><div className="service-lines">{services.map((service)=>{
      const inner=<><span className="service-line-number">{service.number}</span><figure><img src={service.image} srcSet={SRCSET(service.image, 1560)} sizes="(max-width: 768px) 100vw, 560px" alt={service.alt} width={1560} height={600} loading="lazy" decoding="async" /></figure><div className="service-line-copy"><small>{service.label}</small><h2>{service.title}</h2><p>{service.description}</p><b>{service.pricing}</b></div><span className="service-line-arrow">→</span></>
    ...
    <section className="service-process"><figure><img src="/images/renewal/process-tall.webp" srcSet={SRCSET("/images/renewal/process-tall.webp", 1200)} sizes="(max-width: 768px) 100vw, 560px" alt="課題整理から改善までの4段階を上る図解" width={1200} height={1040} loading="lazy" decoding="async" /></figure><div><p className="section-label">Our Approach</p><h2>依頼するサービスより先に、<br />解決する課題を決める。</h2><p>目的と優先順位を揃えることで、制作・導入後も改善を続けられる状態をつくります。</p><ol>{processSteps.map(step=><li key={step.number}><b>{step.number}</b><div><h3>{step.title}</h3><p>{step.text}</p></div></li>)}</ol></div></section>
